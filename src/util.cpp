...
const char * const BITCOIN_CONF_FILENAME = "square.conf";
const char * const BITCOIN_PID_FILENAME = "squared.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "square" is a composite category enabling all Square-related debug output
            if(ptrCategory->count(string("square"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SquareCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SquareCore
    // Mac: ~/Library/Application Support/SquareCore
    // Unix: ~/.squarecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SquareCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SquareCore";
#else
    // Unix
    return pathRet / ".squarecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty square.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override square.conf
